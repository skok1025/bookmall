...
public class Service {
	...
	public static void showMemberList() {
		...
		for (MemberVo vo : list) {
			System.out.printf("%s\t\t%s\t%s\t%s\t%s\n",vo.getNo(),vo.getName(),vo.getTel(),vo.getEmail(),vo.getPasswd());
		}
		UI.listBar();
	}
	...
	public static void showOrderList() {
		...
		UI.header(new String[] {"주문코드","고객명","이메일","결제금액","배송지"});
		for (OrdersVo vo : list) {

			System.out.printf("%s\t%s\t%s\t%s\t\t%s\n",vo.getBusinesscode(),vo.getMemberName(),vo.getMemberEmail(),vo.getPayment(),vo.getDestination());
