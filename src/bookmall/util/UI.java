package bookmall.util;

import java.util.Scanner;

/**
 * 기본 UI를 위한 클래스
 * @author 김석현
 *
 */
public class UI {

	
	public static void mainTitleBar() {
		System.out.println();
		for (int i = 0; i < 40; i++) {
			System.out.print("☆★");
		}
		System.out.println();
	}
	/**
	 * 메뉴 타이틀 이름
	 * @param title 붙일 타이틀
	 */
	public static void menuTitle(String title) {
		System.out.println();
		for (int i = 0; i < 40; i++) {
			System.out.print("●○");
		}		
		System.out.println();
		
		for(int i=0;i<75-title.length()/2;i++) {
			System.out.print(" ");
		}
		System.out.println(title);
		for (int i = 0; i < 40; i++) {
			System.out.print("○●");
		}	
		System.out.println();
	}
	
	/**
	 * 타이틀 이름
	 * @param title
	 */
	public static void title(String title) {
		System.out.println();
		for (int i = 0; i < 18; i++) {
			System.out.print("==");
		}		
		System.out.println();
		
		for(int i=0;i<15-title.length()/2;i++) {
			System.out.print(" ");
		}
		System.out.println(title);
		for (int i = 0; i < 18; i++) {
			System.out.print("==");
		}	
		System.out.println();
	}
	
	/**
	 * 소제목에 붙일 바
	 */
	public static void bar() {
		for (int i = 0; i < 20; i++) {
			System.out.print("--");
		}
		System.out.println();
	}
	
	/**
	 *  리스트 제목에 붙일 바
	 */
	public static void listBar(){
		for (int i = 0; i < 40; i++) {
			System.out.print("==");
		}
		System.out.println();
	}

	/**
	 * 잠시 멈춤
	 */
	public static void pause() {
		System.out.println("뒤로 돌아가려면 아무 키나 눌러주세요.");
		Scanner scan = new Scanner(System.in);
		scan.nextLine();
	}

	/**
	 * 특정 라벨 출력 후 잠시 멈춤
	 * @param label
	 */
	public static void pause(String label) {
		System.out.println(label);
		Scanner scan = new Scanner(System.in);
		scan.nextLine();
	}
	
	/**
	 * 데이터 삽입 전 확인 메소드
	 * @return 데이터 삽입 결정 여부
	 */
	public static boolean inputPause() {
		System.out.print("입력한 내용이 맞습니까?(y/n):");
		Scanner scan = new Scanner(System.in);
		String result = scan.nextLine();
		
		return result.equals("y")?true:false;
	}
	
	/**
	 * 데이터 삭제 전 확인 메소드
	 * @return 데이터 삭제 결정 여부
	 */
	public static boolean deletePause() {
		System.out.print("정말 삭제하시겠습니까?(y/n):");
		Scanner scan = new Scanner(System.in);
		String result = scan.nextLine();
		
		return result.equals("y")?true:false;
	}
	
	/**
	 * 데이터 수정 전 확인 메소드
	 * @return 데이터 수정 결정 여부
	 */
	public static boolean updatePause() {
		System.out.print("정말 수정하시겠습니까?(y/n):");
		Scanner scan = new Scanner(System.in);
		String result = scan.nextLine();
		
		return result.equals("y")?true:false;
	}
	
	/**
	 * 추가,수정,삭제 완료시 메세지 출력 
	 * @param label 
	 */
	public static void completeMessage(String label) {
		System.out.printf("%s (이)가 완료되었습니다.\n",label);
	}
	
	/**
	 * 기본 선택메뉴 출력 메소드
	 * @param label
	 */
	public static void baseSelectOption(String label) { 
		System.out.printf("0.돌아가기  *.처음으로\t\t선택(%s번호):",label);
	}
	/**
	 * 
	 * @param label
	 */
	public static void baseSelectOption2(String label) { 
	      System.out.printf("0.돌아가기  *.처음으로  1.입력\t\t선택(%s번호):",label);
	   }
	
	
	
	/**
	 * 메뉴 선택 리스트 출력 메소드
	 * @param list 메뉴로 들어갈 배열
	 */
	public static void menu(String[] list) {
		System.out.println();
		for (int i = 0; i < list.length; i++) {
			System.out.printf("%d. %s\n", (i + 1), list[i]);
		}
		System.out.println();
	}

	/**
	 * 테이블 출력 > 헤더 출력
	 * @param labels
	 */
	public static void header(String[] labels) {
		for (String label : labels) {
			System.out.printf("[%s]\t\t", label);
		}
		System.out.println();
	}

	/**
	 * 테이블 출력 > 데이터 출력
	 * @param datas 
	 */
	public static void data(Object[] datas) {
		for (Object data : datas) {
			System.out.printf("%s\t", data);
		}
		System.out.println();
	}

	/**
	 * 결과 메시지 출력
	 * @param msg
	 */
	public static void result(String msg) {
		System.out.printf("결과]" + msg);
		System.out.println();
	}

	/**
	 * 결과에 따른 메세지 출력 메소드 
	 * @param result 실행 결과
	 * @param msg 실행 성공시 출력 메세지
	 */
	public static void result(int result, String msg) {
		if (result > 0) {
			System.out.printf("결과]" + msg);
		} else {
			System.out.println("실패했습니다. 관리자에게 문의하세요.");
		}
		System.out.println();
	}

	
	


}
